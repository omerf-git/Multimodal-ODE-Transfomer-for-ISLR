...
class ChaLearnDataModule(pl.LightningDataModule):
    def __init__(self, data_dir=_DATA_DIR_LOCAL, batch_size=16, num_workers=0, sequence_length=16,
                 temporal_stride=1, save_embeddings=False, **kwargs):
        super().__init__()
        ...
        self.use_precomputed = kwargs.get('use_precomputed', True)
    ...
    def train_dataloader(self):
        ...
        self.train_set = ChaLearnDataset(self.data_dir, 'train', 'train',
                                         ...
                                         transform, self.sequence_length, self.temporal_stride, self.use_precomputed)
        
        # Büyük batch size'larda pin_memory problemlerinden kaçınmak için
        use_pin_memory = self.batch_size <= 16
        # Multiprocessing problemlerini önlemek için num_workers'ı sınırla
        safe_num_workers = min(self.num_workers, 2) if self.batch_size > 16 else self.num_workers
        
        return DataLoader(self.train_set, batch_size=self.batch_size, num_workers=safe_num_workers, 
                          pin_memory=use_pin_memory, shuffle=True, persistent_workers=safe_num_workers > 0)
    ...
    def val_dataloader(self):
        ...
        self.val_set = ChaLearnDataset(self.data_dir, 'val', 'val',
                                       os.path.join(self.data_dir, '..', '..', 'train_val_labels_STAGE2.csv'),
                                       transform,
                                       self.sequence_length, self.temporal_stride, self.use_precomputed)
        
        # Büyük batch size'larda pin_memory problemlerinden kaçınmak için
        use_pin_memory = self.batch_size <= 16
        safe_num_workers = min(self.num_workers, 2) if self.batch_size > 16 else self.num_workers
        
        return DataLoader(self.val_set, batch_size=self.batch_size, num_workers=safe_num_workers, 
                          pin_memory=use_pin_memory, persistent_workers=safe_num_workers > 0)
    ...
    def test_dataloader(self):
        ...
        self.test_set = ChaLearnDataset(self.data_dir, 'test', 'test', None, transform, self.sequence_length,
                                        self.temporal_stride, self.use_precomputed)
        
        # Büyük batch size'larda pin_memory problemlerinden kaçınmak için
        use_pin_memory = self.batch_size <= 16
        safe_num_workers = min(self.num_workers, 2) if self.batch_size > 16 else self.num_workers
        
        return DataLoader(self.test_set, batch_size=self.batch_size, num_workers=safe_num_workers, 
                          pin_memory=use_pin_memory, persistent_workers=safe_num_workers > 0)
...
class ChaLearnDataset(Dataset):
    def __init__(self, root_path, job_path, job, label_file_path, transform, sequence_length,
                 temporal_stride, use_precomputed=True):
    ...
    def __getitem__(self, item):
        ...
        if self.use_precomputed:
            # Pre-computed embeddings kullanırken sadece pose data'yı işle
            poseflow_clip = self._load_poseflow_data(sample)
            # Çok küçük dummy RGB clip oluştur (kullanılmayacak)
            dummy_clip = torch.zeros((self.sequence_length, 2, 3, 32, 32), dtype=torch.uint8)  # minimal dimensions
        else:
            # Orijinal video işleme
            self.transform.randomize_parameters()
            dummy_clip, poseflow_clip = self._process_video_frames(sample)
        
        # Sample name'i çıkar
        sample_name = sample['path'].split('/')[-1]
        
        if self.has_labels:
            return (dummy_clip, poseflow_clip), sample['label'], sample_name
        else:
            # Return sample name instead of label so we know what prediction this is
            return (dummy_clip, poseflow_clip), sample['path'].split('/')[-1][:-10], sample_name
    
    def _load_poseflow_data(self, sample):
        """Load only poseflow data when using precomputed embeddings."""
        poseflow_clip = []
        ...
        return torch.stack(poseflow_clip, dim=0)
    
    def _process_video_frames(self, sample):
        """Original video processing method (fallback when embeddings not available)."""
